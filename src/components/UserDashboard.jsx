...
export default function UserDashboard({ token, onLogout, onNavigateTab, onOpenProfile }) {
  ...
  // Estado del Modal de Retiro PIX
  ...
  const fetchDashboard = async () => {
    ...
    try {
      ...
      if (!dashRes.ok) throw new Error('Error al cargar los datos de la Matriz Epi.');
  ...
  const handleWithdrawSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(resData.error || 'Error al solicitar retiro.');
  ...
  if (error) {
    return (
      <div className="glass-card" style={{ textAlign: 'center', padding: '3rem', color: 'var(--accent-rose)' }}>
        <h3>¡Ups! Fallo en la conexión.</h3>
  ...
  const getCycleBadge = (cycleName) => {
    ...
    if (name.includes('Plata') || name.includes('Prata')) return <span className="cycle-badge cycle-plata"><Award size={14} /> Socio Plata</span>;
    if (name.includes('Oro') || name.includes('Ouro')) return <span className="cycle-badge cycle-oro"><Award size={14} /> Socio Oro</span>;
  ...
  const getLayerBadge = (layer) => {
    if (layer === 1) return <span className="level-badge level-1">Capa 1: Maestro</span>;
    if (layer === 2) return <span className="level-badge level-2">Capa 2: Líder</span>;
    return <span className="level-badge level-3">Capa 3: Ayudante</span>;
  };
  ...
  return (
    <div>
      ...
      {/* BANNER DEL ENLACE DE REFERIDO & CICLO CORPORATIVO */}
      <div className="referral-banner">
        <div className="referral-info">
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.5rem', flexWrap: 'wrap' }}>
            ...
            {user.is_active !== false ? (
              <span className="origin-badge origin-direct" style={{ background: 'rgba(16, 185, 129, 0.2)', color: '#34d399', borderColor: 'rgba(16, 185, 129, 0.4)' }}>
                ✅ Cuenta Activa (Activación al día)
              </span>
            ) : (
              <span className="origin-badge origin-spillover" style={{ background: 'rgba(244, 63, 94, 0.2)', color: '#f43f5e', borderColor: 'rgba(244, 63, 94, 0.4)' }}>
                ⚠️ Cuenta Inactiva (Activación Pendiente)
              </span>
            )}
          ...
          <h3>Tu Enlace de Referido Epi (Base 3)</h3>
          <p>Invita a nuevos afiliados con tu enlace directo. Las indicaciones excedentes entran automáticamente por Desbordamiento.</p>
        </div>
      ...
      {/* BANNER ACCESO RÁPIDO A ACADEMIA LMS / CURSOS */}
      <div className="glass-card" style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <div>
            ...
            <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem', marginTop: '0.2rem', margin: 0 }}>
              Mira video-clases prácticas, capacitaciones de liderazgo y materiales de apoyo del sistema Epi.
            </p>
          </div>
        </div>
        <button
        ...
        >
          <BookOpen size={16} /> Acceder a Cursos
        </button>
      </div>

      {/* BILLETERA DIGITAL Y RETIROS PIX */}
      <div className="glass-card" style={{ marginBottom: '1.5rem', border: '1px solid var(--primary-glow)' }}>
        <div className="card-header">
          <div className="card-title">
            ...
            <span>Billetera Digital (Wallet & Comisiones)</span>
          </div>
          <button
            type="button"
            className="nav-btn nav-btn-primary"
          ...
          >
            <Send size={16} /> Solicitar Retiro PIX
          </button>
        </div>
        ...
        <div className="stats-grid" style={{ marginBottom: '1rem' }}>
          <div className="stat-card stat-card-emerald">
            ...
            <div className="stat-subtext">Listo para solicitud de retiro PIX</div>
          </div>

          <div className="stat-card stat-card-purple" style={{ borderLeftColor: '#fbbf24' }}>
            <div className="stat-header">
              <span className="stat-title">Saldo en Procesamiento</span>
            ...
            <div className="stat-subtext">Esperando aprobación del pago</div>
          </div>

          <div className="stat-card stat-card-indigo">
            <div className="stat-header">
              <span className="stat-title">Total Ya Retirado / Ganado</span>
            ...
            <div className="stat-subtext">Acumulado histórico de comisiones</div>
          </div>
        </div>

        {/* TABLA DE RETIROS DEL AFILIADO */}
                ...
                <tbody>
                  {walletData.withdrawals.map((w) => (
                    <tr key={w.id}>
                      ...
                      <td>
                        {w.status === 'approved' ? (
                          <span className="origin-badge origin-direct">✅ Pago (Aprobado)</span>
                        ) : w.status === 'rejected' ? (
      ...
      <div className="glass-card" style={{ marginBottom: '1.5rem' }}>
        <div className="card-header">
          <h3 style={{ fontSize: '1rem', fontWeight: 700, color: '#fff' }}>📊 Resumen de Mi Red</h3>
        </div>
      ...
      <div className="refund-card" style={{ flexDirection: 'column', alignItems: 'stretch', gap: '1rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <h4 style={{ fontSize: '1.1rem', fontWeight: 800, color: '#fff' }}>
              Progresión de Ciclos — {cyclesData?.current_cycle?.display_name || user.current_cycle}
            </h4>
            <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem', marginTop: '0.2rem' }}>
              Precio: $US {cyclesData?.current_cycle?.price ?? '—'} | Bono por referido: $US {cyclesData?.current_cycle?.bonus_per_referral ?? '—'}
            </p>
          </div>
          {cyclesData?.next_cycle && (
            <button
              ...
              onClick={async () => {
                if (!window.confirm(`Hacer upgrade para ${cyclesData.next_cycle.display_name} ($US ${cyclesData.next_cycle.price})?`)) return;
                try {
        ...
        {/* Barra de progreso de los 5 ciclos */}
        <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
          {['Bronze', 'Plata', 'Oro', 'Platino', 'Diamante'].map((name, idx) => {
        ...
        {/* Status de reembolso del ciclo actual */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.75rem', background: 'rgba(15,23,42,0.6)', borderRadius: '8px' }}>
          <div>
            <span style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>
               Reembolso del Ciclo {cyclesData?.current_cycle?.name || 'Bronze'}:
            </span>
              ...
              <tbody>
                {transactions.slice(0, 10).map((tx) => (
                  <tr key={tx.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                    <td style={{ padding: '0.5rem', color: 'var(--text-muted)' }}>{new Date(tx.created_at).toLocaleDateString('es-ES')}</td>
                    <td style={{ padding: '0.5rem' }}>
                      <span style={{ fontSize: '0.78rem', padding: '0.15rem 0.5rem', borderRadius: '4px',
                        ...
                        color: tx.type === 'commission_direct' ? '#34d399' :
                         ...
                         {tx.type === 'commission_direct' ? '💰 Comisión' :
      ...
      {/* PROGRESO DE LAS 3 CAPAS DE LA MATRIZ Epi (3, 9, 27 = 39) */}
      <div className="epi-layers-grid">
        <div className="layer-card">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: '0.85rem', fontWeight: 700, color: 'var(--text-muted)' }}>Capa 1: Maestros</span>
          ...
          <div style={{ fontSize: '1.8rem', fontWeight: 800, color: '#fff', marginTop: '0.5rem' }}>
            {epi_layers.maestros.count} <span style={{ fontSize: '0.9rem', color: 'var(--text-subtle)' }}>miembros</span>
          </div>
        ...
        <div className="layer-card">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: '0.85rem', fontWeight: 700, color: 'var(--text-muted)' }}>Capa 2: Líderes</span>
            <span className="level-badge level-2">{epi_layers.lideres.count} / 9</span>
          </div>
          <div style={{ fontSize: '1.8rem', fontWeight: 800, color: '#fff', marginTop: '0.5rem' }}>
            {epi_layers.lideres.count} <span style={{ fontSize: '0.9rem', color: 'var(--text-subtle)' }}>miembros</span>
          </div>
          <div className="layer-progress-bar">
        ...
        <div className="layer-card">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: '0.85rem', fontWeight: 700, color: 'var(--text-muted)' }}>Capa 3: Ayudantes</span>
            <span className="level-badge level-3">{epi_layers.ayudantes.count} / 27</span>
          </div>
          <div style={{ fontSize: '1.8rem', fontWeight: 800, color: '#fff', marginTop: '0.5rem' }}>
            {epi_layers.ayudantes.count} <span style={{ fontSize: '0.9rem', color: 'var(--text-subtle)' }}>miembros</span>
          </div>
          <div className="layer-progress-bar">
        ...
        <div className="layer-card">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: '0.85rem', fontWeight: 700, color: 'var(--accent-amber)' }}>Progreso de la Plataforma</span>
      ...
      <div className="glass-card">
        <div className="card-header">
          <div className="card-title">
            ...
            <span>Miembros de la Matriz Epi ({matrix.length} / 39)</span>
          </div>
          ...
          <div>
            {matrix.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '3rem', color: 'var(--text-muted)' }}>
                ...
                <p style={{ fontWeight: 600 }}>Tu matriz 3x3 aún no tiene miembros registrados.</p>
                <p style={{ fontSize: '0.85rem', marginTop: '0.4rem' }}>
                  Invita a tus 3 primeros invitados con tu enlace para llenar tu línea de Maestros!
                </p>
              </div>
            ) : (
              <div className="table-responsive">
                  ...
                  <tbody>
                    {matrix.map((item) => (
                      <tr key={item.id}>
                        ...
                        <td>
                          {item.is_spillover ? (
                            <span className="origin-badge origin-spillover">🌊 Desbordamiento</span>
                          ) : (
                            <span className="origin-badge origin-direct">🎯 Directo</span>
                          )}
                        </td>
                        <td style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>
      ...
      {/* MODAL DE SOLICITUD DE RETIRO PIX */}
      {showWithdrawModal && (
        <div style={{
          ...
          <div className="glass-card" style={{ maxWidth: '440px', width: '100%', border: '1px solid var(--primary-glow)', background: '#0f172a' }}>
            ...
            <div style={{ background: 'rgba(56, 189, 248, 0.1)', border: '1px solid rgba(56, 189, 248, 0.3)', borderRadius: '8px', padding: '0.85rem', marginBottom: '1.2rem', color: '#67e8f9', fontSize: '0.88rem' }}>
              Saldo Disponible: <strong>$US {walletData.balance.toFixed(2)}</strong>
            </div>
            ...
            <form onSubmit={handleWithdrawSubmit}>
              <div className="form-group">
                <label className="form-label">Monto a Retirar ($US)</label>
              ...
              <div className="form-group">
                <label className="form-label">Clave PIX para Recepción</label>
                <div className="input-wrapper">
                  <CreditCard className="input-icon" size={18} />
                  <input
                    type="text"
                    className="form-input"
                    placeholder="CPF, E-mail, Teléfono o Clave Aleatoria"
              ...
              <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'flex-end', marginTop: '1.5rem' }}>
                ...
                <button type="submit" className="nav-btn nav-btn-primary" disabled={withdrawLoading || walletData.balance <= 0}>
                  {withdrawLoading ? 'Enviando Solicitud...' : 'Confirmar Retiro PIX'}
                </button>
              </div>
            </form>
          </div>
